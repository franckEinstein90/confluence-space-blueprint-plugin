...
AJS.toInit(function($) {
  function submitExampleSpace(e, state) {
      /*setting up the title of the home page*/
      state.pageData.ContentPageTitle =  AJS.I18n.getText("confluence.hello.blueprint.space.home.title.prefix") +
          " " + state.pageData.name + " " +
          AJS.I18n.getText("confluence.hello.blueprint.space.home.title.suffix");
...
function postRenderSpaceWizard(e, state){
/*	var item = document.getElementById("decisions-form").firstElementChild.firstElementChild.firstElementChild;
  var textnode = document.createTextNode("project Number");
  ...
  return Confluence.SpaceBlueprint.CommonWizardBindings.postRender(e, state);
    ...
    Confluence.Blueprint.setWizard('com.atlassian.confluence.plugins.hello-blueprint:hello-space-blueprint-item', function(wizard) {
      wizard.on("submit.spaceBasicDetailsId", submitExampleSpace);
      wizard.on("pre-render.spaceBasicDetailsId", preRenderExampleSpace);
      wizard.on("post-render.spaceBasicDetailsId", postRenderSpaceWizard);
    });
